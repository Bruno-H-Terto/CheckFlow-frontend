...
export function App() {
  ...
  useEffect(() => {
    ...
    const connection = new WebSocket(
      `${WS_URL}/ws/plans/${selectedId}/executions`
    );
    ...
    connection.onmessage = (event) => {
      setEvents((current) =>
        [JSON.parse(event.data) as RealtimeEvent, ...current].slice(0, 30)
      );
  ...
  async function createStep(payload: StepCreate) {
    if (selectedId === null) return;

    try {
      await api.createStep(selectedId, payload);
  ...
  async function updateStep(stepId: number, payload: StepUpdate) {
    if (selectedId === null) return;

    try {
      await api.updateStep(selectedId, stepId, payload);
      await loadPlanData(selectedId);
    } catch (cause) {
      setError(message(cause));
      throw cause;
  ...
  async function deleteStep(stepId: number) {
    ...
    try {
      await api.deleteStepe(selectedId, stepId);
      await loadPlanData(selectedId);
    } catch (cause) {
      throw cause;
    }
  ...
  return (
    <main className="mx-auto w-full max-w-[1440px] p-4 md:p-7">
      ...
      {error && (
        <button
          className="mb-5 w-full rounded-lg border border-red-300 bg-red-50 p-3 text-left text-red-800"
          onClick={() => setError("")}
        >
          {error} ×
        </button>
          ...
          {!selectedPlan ? (
            ...
            <>
              <section className={`${panel} flex flex-col items-start justify-between gap-4 bg-gradient-to-br from-white to-emerald-50 sm:flex-row sm:items-center`}>
                ...
                <button
                  data-cy="execute-plan"
                  className={primaryButton}
                  onClick={() => run(() => api.executePlan(selectedPlan.id))}
                >
                  Executar plano
                </button>
              </section>
              ...
              <div className="grid items-start gap-5 xl:grid-cols-2">
                ...
                <ExecutionsPanel
                  ...
                  onCancel={(id) =>
                    run(() => api.cancelExecution(selectedPlan.id, id))
                  }
                  onRetry={(id) =>
                    run(() => api.retryExecution(selectedPlan.id, id))
                  }
                />
              </div>
